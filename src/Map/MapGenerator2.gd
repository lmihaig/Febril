...
func point_from_circle_coords(length, angle):
	# angle in radians
...
func generate(plane_len, node_count, path_count):
	...
	for i in range(node_count):
		while true:
			var length=rand_range(1,6);
			var angle=rand_range(0.3,0.7)*PI;
			var point= point_from_circle_coords(length, angle);
...
#				 and in_circle:
	...
	var data = preload("res://src/Map/MapData.gd").new()
